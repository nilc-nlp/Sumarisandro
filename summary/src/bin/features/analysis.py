...
class FeatureSalienceCV(BaseEstimator, MetaEstimatorMixin):
    ...
    def fit_mix(self, X, y):
            ...
            for feat in feature_names:
                ...
                if self.score_func is None:
                    ...
                    for score_func in self.score_func:
                        ...
                        scores[feat][score_func_name] += score_func(y[test], estimator.predict(feat_encoder.transform(selected_X[test])))
        ...
        if self.verbose:
            ...
            scores_list = [ (feat_name, feat_scores[self.sort_by]) for feat_name, feat_scores in self.cv_scores_.iteritems() ]
            scores_list.sort(key=lambda (k,v):v, reverse=True)
             
            print "\nFeature Salience in Descending Order of {} for {}:".format(self.sort_by.upper(), self.estimator.__class__.__name__)
    ...
    def fit_mix(self, X, y):
        ...
        for train, test in cv:
            ...
            for feat in self.feature_order:
                ...
                if self.verbose > 0:
                    progress_line = "{:>4}\t{:<25}".format(n+1,feat)
                    progress_line+="\t{:>5.1f}\t{:>13}/{}".format(score_fold*100,self.feature_order.index(feat) +1, len(self.feature_order))
                    print progress_line
        ...
        # Report the final results
        if self.verbose > 0:
            print "\nFeature performance analysed by {} for {}:".format(self.score_func.__name__, self.estimator.__class__.__name__)

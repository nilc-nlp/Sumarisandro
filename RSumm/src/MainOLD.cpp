...
//////////////////////////////////////////////////////////////// 
// CHANGE SOME SENTENCES THAT 'SENTER' SPLITTED!!!!!!!!!!!!!!!!!!!!
//getchar();
//////////////////////////////////////////////////////////////// 
...
//--------------------------------------------------------------
		...
		vector<Sentence*> *vec_segmentado = salton->caminhoSegmentado(total, desempate, (char*)"manual");
...
FILE *arq_summary_seg = fopen("summary.segmentado", "w+");

	if(!arq_summary_seg){

		cout << endl << "ERROR! Summary Archive (Segmented)" << endl << endl;
		exit(EXIT_FAILURE);		
	}
...
cout << "\n\n--Final Summary--\n";
		...
		for(int i = 0; i < vec_segmentado->size(); i++){

			Sentence *s = vec_segmentado->at(i);
			char **auxiliar = s->getSentencaOriginal();

			for(int j = 0; j < s->getNumPalavras(); j++){

				fprintf(arq_summary_seg, "%s ", auxiliar[j]);
				cout << auxiliar[j] << " ";
				aux_taxa_compressao++;
			}

			fprintf(arq_summary_seg, "\n");
			cout << endl;
		}

		fclose(arq_summary_seg);

int intersect = 0;
int num_sent_ext = Util().numSentExt(vec_segmentado, &intersect);
float f_measure = Util().calcularF_measure(intersect, vec_segmentado->size(), num_sent_ext);
float precision = Util().precisionRecall(intersect, num_sent_ext);
float recall = Util().precisionRecall(intersect, vec_segmentado->size());

FILE *arq_p = fopen("pre", "a");
FILE *arq_r = fopen("rec", "a");
FILE *arq_f = fopen("fme", "a");

if(!arq_p || !arq_r || !arq_f){

	cout << endl << "ERROR! PRF Archive" << endl << endl;
	exit(EXIT_FAILURE);		
}
cout << "Num_sent_extrato: " << num_sent_ext;
